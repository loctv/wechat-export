...
MAX_GROUP_NUM = 2  # each number
INTERFACE_CALLING_INTERVAL = 5  #  interface call time interval, the interval is too short prone to "so frequently," will be limited to operate about half an hour
MAX_PROGRESS_LEN = 50
...
def showQRImage():
    ...
    print('Please use Discover in mobile scan to login')
...
def waitForLogin():
    ...
    if code == '201':  # scanned
        print('Successfully scanned, click OK to log in on the phone')
        tip = 0
    elif code == '200':  # logged
        print('Signing in...')
...
def webwxgetcontact():
    ...
    # Reverse traversal, or deleted when a problem...
...
def addMember(ChatRoomName, UserNames):
    ...
    BlockedList = []
    for Member in MemberList:
        if Member['MemberStatus'] == 4:  # one was deleted
            DeletedList.append(Member['UserName'])
        elif Member['MemberStatus'] == 3:  # blacklisted
            BlockedList.append(Member['UserName'])
...
def main():
    ...
    print('Getting the two-dimensional code image...')
    showQRImage()
    ...
    print('open heartbeat thread')
    threading.Thread(target=heartBeatLoop)

    MemberCount = len(MemberList)
    print('% s address book altogether friends' % MemberCount)
    ...
    for i in range(0, group_num):
        ...
        # todo BlockedList pulled blacklist
        ...
        print('You are %d newly discovered Remove' % DeletedCount)
        ...
        if i != group_num - 1:
            print('is continuing to look, please wait ...')
            # 下一次进行接口调用需要等待的时间
            time.sleep(INTERFACE_CALLING_INTERVAL)
    # todo Delete group

    print('\ n summary of the results is completed, 20s after retry ..')
    resultNames = []
    for r in result:
        if d[r][1] != '':
    ...
    print('---------- deleted buddy list (%d total people) ----------' % len(result))
    # Filter emoji
...
if __name__ == '__main__':
    ...
    print('The results of this program may cause some psychological discomfort, use caution...')
    print('can only be used once within an hour, otherwise they will be a busy operation to prevent colonization')
    main()
